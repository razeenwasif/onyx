...
**Editing-polish features (2026-06-14).** Several Obsidian/Notion-staple features layer onto the model above: **aliases** (`aliases:` frontmatter indexed as extra basenames in `NoteIndex.by_basename`), **clickable outline** (`App::outline_headings`/`jump_to_heading`), **`#tag` autocomplete** (`App::tag_complete`, mirrors `[[`/`/`), **task toggle** (`t` / `:task`, `parse::toggle_task_marker`), **safe rename** (`Vault::rename_with_backlinks` + `parse::rename_link_targets`), **`:tasks`** rollup (`parse::task_line` + `ui/tasks.rs`), **bookmarks** (`App.bookmarks` → `.onyx/bookmarks.json`, Home + `★`), **editable kanban** (`App::board_move_card` rewrites the group-by property via `parse::set_frontmatter_property`), **editor tabs** (`App.{tab_paths, tabs}` keep `doc` as the active tab so existing accessors are unchanged; `ui/tabline.rs`), **inline property editing** (`:props` → `Focus::Properties`/`PropsEditState`/`ui/props.rs`, buffer-based so it joins undo+save), and **split view** (`App.split_doc` → `ui/splitview.rs` renders a second note read-only in the right pane).

**Local AI assistant (2026-06-14).** A streaming chat over a local LLM via Ollama, behind the **`ai`** cargo feature (`full = ["cloud","ai"]`). `src/integrations/ollama.rs` mirrors the cloud pattern: pure request/response shaping (`chat_body`, `parse_chat_chunk` — these Gemma builds stream a `thinking` trace separate from `content` — `parse_models`) always compiled + unit-tested; the streaming HTTP (`chat_stream`, NDJSON via blocking `reqwest` + `BufRead`, `AtomicBool` cancel) and `list_models` behind `#[cfg(feature="ai")]` with non-ai stubs. `Focus::Ai` (`ui/ai.rs`, `App.ai: AiState` of `AiTurn`s) is a live text field; submitting spawns `ai_worker` which streams `AiMsg::{Delta,Done,Err}` over mpsc (epoch-tagged, `ai_cancel: Arc<AtomicBool>`), drained each tick by `drain_ai` (appends deltas to the in-progress assistant turn; `ai_streaming()` joins the fast-poll). The open note is sent as the system-context message. `Ctrl-A`/`:ai` open it; `:ai <prompt>`, `:summarize`, `:ai model <n>`/`:ai models`/`:ai clear`. Config: `[ai] model` (default `gemma4:e4b-it-qat`) + `host`. See `docs/AI.md`.

**Inline autocomplete / ghost text (2026-06-14).** In insert mode, `App::note_edit` (called after every insert key in `editor_keys`) stamps `last_edit` and drops any stale `App.ghost`. `maybe_request_ghost` (each tick) fires a debounced (`GHOST_DEBOUNCE` 600 ms) completion when idle at a line end — spawning `ghost_worker` (fast `[ai] completion_model`) tagged with `ghost_epoch`/`ghost_cancel` and the buffer `revision` (`ghost_for_rev`). `drain_ghost` shows the result only if the revision still matches; `ui/editor_pane.rs` appends it dimmed after the cursor. `Tab` → `accept_ghost` (insert at cursor); `ghost_armed`/`ghost_pending` keep the event loop polling fast so it can fire/stream without further input. `:ai complete on|off` toggles `[ai] autocomplete`.

**Editor Visual selection (2026-06-14).** Line-wise Visual mode: `v`/`V` (handled before the doc borrow in `editor_normal`) set `doc.anchor` + `Mode::Visual`; `dispatch::editor_visual` handles it — motions extend, `y`/`d`/`x` yank/delete (`App::visual_yank`/`visual_delete` → `App.register`, line-wise via `Buffer::remove_line_range`), `r` → `rewrite_selection`, `v`/`Esc` exit (Esc via `App::escape`'s Editor arm). `p`/`P` (`paste_register` → `Buffer::insert_lines`) paste in normal mode. Selected lines get `patch_style(s_selection())` in `ui/editor_pane.rs`. The `:rewrite` command routes through `App::rewrite_command`, which prefers the active Visual range, else paragraph/whole-note, all funnelling into the shared `rewrite_lines`. The AI chat system prompt also embeds `keymap::cheatsheet()` so the assistant can answer "what's the shortcut for…".

**AI rewrite in place (2026-06-14).** `:rewrite [instr]` (`:rewrite all` = whole note) AI-rewrites the paragraph around the cursor and replaces it in one undo-able edit. `Buffer::replace_line_range(start,end,text)` splices the (multi-line) result; `App::rewrite_range` picks the target (paragraph = contiguous non-blank lines around the cursor, or the whole note), builds an "output ONLY the rewritten text" prompt, and runs the chat `ai_worker` into a separate `rewrite_rx`/`rewrite_epoch`/`rewrite_cancel`; `drain_rewrite` accumulates `RewriteState.acc` then on completion records undo + `replace_line_range` + dirty (`clean_rewrite_output` strips a stray surrounding ``` fence). Editor has no visual selection yet, so the target is paragraph/note, not an arbitrary span.

**Ask my vault — RAG (2026-06-14).** `:ask <q>` answers from the whole vault by semantic search. `src/rag.rs` is pure + tested: `chunk_note` (merge paragraphs to ~1000 chars, skip frontmatter), `cosine`, `top_k`, and the `.onyx/rag-index.json` cache (`RagIndex` keyed by abs-path → `NoteEmbeds{mtime, chunks:[EmbeddedChunk{text,line,vec}]}`). `integrations::ollama::embed` (batch `/api/embed`, needs an embedding model — `nomic-embed-text` default in `[ai] embed_model`) does the vectors. `rag_worker` loads the cache, re-embeds only mtime-changed notes (drops removed; resets if the embed model changed), saves, embeds the query, ranks top-K; it streams `RagMsg::{Progress,Ready,Err}` (epoch/`rag_gen` cancel) drained by `drain_rag`, which shows indexing progress in the AI overlay title then calls `start_rag_answer` to build a grounded system prompt and `begin_stream` the reply (`ai_pending_sources` appends a `— Sources:` line when it finishes). First ask indexes the whole vault (cached after).

**Obsidian-feel bundle (2026-06-14).** Three more feel items: **unlinked mentions** — the Backlinks pane (`ui/sidebar.rs::draw_backlinks`) appends notes that mention the open note's name/aliases in plain text without linking it. Computed on a background worker (`unlinked_worker` + `contains_word` whole-word match in `app.rs`, same epoch/`Arc<AtomicU64>` cancellation as search), cached in `App.unlinked` (`UnlinkedState`), refreshed each tick when the open note changes (`maybe_refresh_unlinked`/`drain_unlinked`, gated on `show_right`); rows come from `App::backlink_rows` (real backlinks then `~` mentions, both openable). **Search operators** — `parse_search_query` turns the query into a `SearchQuery` with `tag:`/`path:`/`line:N`; `tag:`/`path:` pre-filter the candidate files via the index (`filtered_search_paths`), free text still line-scans in `search_worker`, and a filters-only query lists one hit per matching note. **Scrollable help** — `App.help_scroll` + a windowed `ui/help.rs` (title shows `start–end/total`; clamps in the renderer), scrolled by `help_keys` (`j/k`, `d/u`, `g/G`, arrows).

**Notion export import (Phase 5).** `:notion import <folder>` (an *unzipped* Notion "Markdown & CSV" export) is handled by `src/notion_import.rs` — pure helpers (`clean_component` strips the ` <32-hex>` Notion ids, `parse_csv`, `rewrite_links` turns internal `.md` links into `[[wikilinks]]` and cleans attachment paths, `csv_row_frontmatter`) plus a two-pass `import_export`: pass 1 writes a `_schema.md` per CSV database and builds a row→frontmatter map; pass 2 rewrites each note's links, injects the matching database row's columns as YAML frontmatter, and copies attachments. `App::import_notion` runs it under `<vault>/Notion Import/` (create-only, collisions kept as ` (Notion)`) and refreshes the index. No network — see the BACKLOG note on why the export route was chosen over a live API client.
